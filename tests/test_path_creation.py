...
def test_create_root_node_automatic(db):
    ...
    root = Taxonomy.t_objects.create_child(name="test_auto_root")
...
def test_create_children_automatic(db):
    ...
    # Create children
    for i in range(3):
        Taxonomy.t_objects.create_child(name=f"test_auto_child_{i}", parent=root)
...
def test_children_have_automatic_labels(db):
    ...
    root = Taxonomy.t_objects.create_child(name="test_auto_root")
    child = Taxonomy.t_objects.create_child(name="test_auto_child", parent=root)
...
def test_multilevel_hierarchy_automatic(db):
    ...
    root = Taxonomy.t_objects.create_child(name="test_auto_root")
    child = Taxonomy.t_objects.create_child(name="test_auto_child", parent=root)
    grandchild = Taxonomy.t_objects.create_child(
        name="test_auto_grandchild", parent=child
    )
...
def test_deep_tree_automatic(db):
    ...
    current = root
    for i in range(5):
        current = Taxonomy.t_objects.create_child(
            name=f"test_auto_level_{i}", parent=current
        )
...
def test_create_root_with_custom_label(db):
    ...
    root = Taxonomy.t_objects.create(name="test_manual_root", label="ROOT")
...
def test_create_children_with_custom_labels(db):
    ...
    labels = ["A", "B", "C"]
    for label in labels:
        Taxonomy.t_objects.create_child(
            name=f"test_manual_child_{label}", label=label, parent=root
        )
...
def test_hierarchy_with_custom_labels(db):
    ...
    root = Taxonomy.t_objects.create(name="test_manual_root", label="ROOT")
    child = Taxonomy.t_objects.create_child(
        name="test_manual_child", label="CHILD", parent=root
    )
...
def test_path_construction_with_custom_labels(db):
    ...
    root = Taxonomy.t_objects.create(name="test_manual_root", label="R")
    child = Taxonomy.t_objects.create_child(
        name="test_manual_child", label="C", parent=root
    )
    grandchild = Taxonomy.t_objects.create_child(
        name="test_manual_grandchild", label="G", parent=child
    )
...
def test_retrieve_node_by_path(db):
    ...
    # Retrieve by path
    node = Taxonomy.t_objects.filter(path="R.C.G").first()
