...
def test_children(db, name, expected):
    create_test_data()
    children = (
        Taxonomy.t_objects.get(name=name).children().values_list("name", flat=True)
    )
...
def test_ancestors(db, name, expected):
    create_test_data()
    ancestors = (
        Taxonomy.t_objects.get(name=name).ancestors().values_list("name", flat=True)
    )
...
def test_descendants(db, name, expected):
    create_test_data()
    descendants = (
        Taxonomy.t_objects.get(name=name).descendants().values_list("name", flat=True)
    )
...
def test_siblings(db, name, expected):
    create_test_data()
    siblings = (
        Taxonomy.t_objects.get(name=name).siblings().values_list("name", flat=True)
    )
